...
public class ProgressWheel extends View {
    ...
    private void parseAttributes(TypedArray a) {
        barWidth = (int) a.getDimension(R.styleable.ProgressWheel_barWidthProgress,
                barWidth);

        rimWidth = (int) a.getDimension(R.styleable.ProgressWheel_rimWidthProgress,
                rimWidth);

        spinSpeed = (int) a.getDimension(R.styleable.ProgressWheel_spinSpeedProgress,
                spinSpeed);

        delayMillis = a.getInteger(R.styleable.ProgressWheel_delayMillisProgress,
                delayMillis);
        if (delayMillis < 0) {
            delayMillis = 0;
        }

        barColor = a.getColor(R.styleable.ProgressWheel_barColorProgress, barColor);

        barLength = (int) a.getDimension(R.styleable.ProgressWheel_barLengthProgress,
                barLength);

        textSize = (int) a.getDimension(R.styleable.ProgressWheel_textSizeProgress,
                textSize);

        textColor = (int) a.getColor(R.styleable.ProgressWheel_textColorProgress,
                textColor);

        //if the text is empty , so ignore it
        if (a.hasValue(R.styleable.ProgressWheel_textProgress)) {
            setText(a.getString(R.styleable.ProgressWheel_textProgress));
        }

        rimColor = (int) a.getColor(R.styleable.ProgressWheel_rimColorProgress,
                rimColor);

        circleColor = (int) a.getColor(R.styleable.ProgressWheel_circleColorProgress,
                circleColor);

        contourColor = a.getColor(R.styleable.ProgressWheel_contourColorProgress, contourColor);
        contourSize = a.getDimension(R.styleable.ProgressWheel_contourSizeProgress, contourSize);
